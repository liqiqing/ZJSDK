...
#define TXAdJMLog( s, ... ) NSLog( @"[%@:%d] %@", [[NSString stringWithUTF8String:__FILE__] \
 ...
 *
 */
@protocol TXAdOptional
@end

/**
 * Make all objects compatible to avoid compiler warnings
 */
@interface NSObject (TXAdJSONModelPropertyCompatibility) <TXAdOptional, TXAdIgnore>
@end
 ...
 */
@protocol TXAdAbstractJSONModelProtocol <NSCopying, NSCoding>
 ...
 */
@interface TXAdJSONModel : NSObject <TXAdAbstractJSONModelProtocol, NSSecureCoding>
...
+ (void)setGlobalKeyMapper:(TXAdJSONKeyMapper *)globalKeyMapper DEPRECATED_MSG_ATTRIBUTE("override +keyMapper in a base model class instead");
 ...
 */
- (instancetype)initWithString:(NSString *)string error:(TXAdJSONModelError **)err;
 ...
 * or a property type in your model is not supported by JSONValueTransformer and its categories
 */
- (instancetype)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(TXAdJSONModelError **)err;
